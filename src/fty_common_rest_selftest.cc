/*  =========================================================================
    fty_common_rest_selftest.c - run selftests
...
{
    test_item_t *item;
    printf ("Running fty-common-rest selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef FTY_COMMON_REST_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_common_rest_private_selftest (verbose, item->subtest);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("fty_common_rest_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running fty-common-rest test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FTY_COMMON_REST_BUILD_DRAFT_API // selftest is still in draft
        else
            fty_common_rest_private_selftest (verbose, test->subtest);
#endif // FTY_COMMON_REST_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
