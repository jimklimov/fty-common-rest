...
if [ -d "$FOLDER_NAME" ]; then
    ...
    if [ "x$REQUESTED_BRANCH" = "x" ]; then
        echo "git clone https://github.com/42ity/tntnet.git $FOLDER_NAME"
        $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/tntnet.git $FOLDER_NAME
    else
        if git ls-remote --heads https://github.com/42ity/tntnet.git | grep -q "$REQUESTED_BRANCH"; then
            echo "git clone -b "$REQUESTED_BRANCH" https://github.com/42ity/tntnet.git $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 -b "$REQUESTED_BRANCH" https://github.com/42ity/tntnet.git $FOLDER_NAME
        else
            echo "$REQUESTED_BRANCH not found for https://github.com/42ity/tntnet.git"
            echo "git clone https://github.com/42ity/tntnet.git $FOLDER_NAME"
            $CI_TIME git clone --quiet --depth 1 https://github.com/42ity/tntnet.git $FOLDER_NAME
        fi
    fi
