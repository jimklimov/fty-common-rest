...
if [ "$BUILD_TYPE" == "default" -a -n "${GH_TOKEN}" -a -n "${OBS_STABLE_TOKEN}" -a -n "${OBS_DRAFT_TOKEN}" ]; then
    ...
    TAG_SHA=$(curl -s -H "Authorization: token ${GH_TOKEN}" -X GET https://api.github.com/repos/42ity/fty-common-rest/git/refs/tags/${TRAVIS_TAG} | grep -o -P '(?<=sha":\s).*(?=,)')
    curl -H "Authorization: token ${GH_TOKEN}" -X POST --data "{\"ref\":\"refs/heads/tmp_obs_release_branch\",\"sha\":${TAG_SHA}}" https://api.github.com/repos/42ity/fty-common-rest/git/refs
    ...
    sleep 60
    curl -H "Authorization: token ${GH_TOKEN}" -X DELETE https://api.github.com/repos/42ity/fty-common-rest/git/refs/heads/tmp_obs_release_branch
fi
