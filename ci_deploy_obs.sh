...
if [ "$BUILD_TYPE" == "default" -a -n "${GH_TOKEN}" -a -n "${OBS_STABLE_TOKEN}" -a -n "${OBS_DRAFT_TOKEN}" ]; then
    ...
    curl -H "Authorization: token ${GH_TOKEN}" -X DELETE https://api.github.com/repos/42ity/fty-common-rest/git/refs/heads/latest_release
    curl -H "Authorization: token ${GH_TOKEN}" -X POST --data "{\"ref\":\"refs/heads/latest_release\",\"sha\":${TAG_SHA}}" https://api.github.com/repos/42ity/fty-common-rest/git/refs
